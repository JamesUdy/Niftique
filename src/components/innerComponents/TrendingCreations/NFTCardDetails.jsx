  ...
  return (
    <section className='relative mx-3 sm:mx-6 p-1 h-[460px] my-8 group flex flex-col justify-between bg-[#0f0421bd] ring-2 ring-[#23153f] rounded-xl'>
      <div className="rounded-xl overflow-hidden m-1">
        <img loading='lazy' src={data.img} alt={data.userName} className='w-full h-full transform group-hover:scale-125 ease-in-out transition-transform duration-500' />
      </div>
      <div className='w-full flex flex-col space-y-1 px-3 py-1'>
          ...
          <div className='flex items-center space-x-1'>
            ...
            {data.status && <img loading='lazy' src={Checked} alt='UserName Verified' className='w-4 h-4' />}
          </div>
        </div>
        <div className='px-2 py-2 mb-4 flex justify-between items-center'>
          <div className='flex items-center space-x-2'>
            <img loading='lazy' src={data.userDp} alt="User Profile Photo" className='w-8 h-8 rounded-full ring-2 ring-[#3e207b]' />
